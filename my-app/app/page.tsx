...
export default function Home() {
  ...
  async function handleSummarise(forceLength?: 'short' | 'medium' | 'long', forceRegenerate = false) {
    if (!selected) return;
    ...
    try {
      ...
      setSummaryFromCache(data.cached === true);
    } catch (e: unknown) {
                        ...
                        <div className="rounded-xl border border-purple-100 bg-purple-50 p-4">
                          <div className="flex items-center justify-between gap-2 mb-3">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {summaryFromCache ? (
                                <span className="text-xs px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 border border-amber-200">⚡ Cached</span>
                              ) : (
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                              <button
                                onClick={() => handleSummarise(undefined, true)}
                                disabled={loadingSummary}
                                title="Regenerate summary using AI"
                                className="flex items-center gap-1 text-xs text-gray-500 hover:text-purple-700 border border-gray-200 hover:border-purple-300 rounded-md px-2 py-1 transition-colors disabled:opacity-40"
                              >
